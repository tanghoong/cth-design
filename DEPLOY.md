...
## URL form — worth knowing

`html_handling: "auto-trailing-slash"` (the default, and what is deployed)
strips the extension:

```
/pages/brand.html  →  307  →  /pages/brand  →  200
```

So the **public** URL of every page is extensionless. `canonical`, `og:url` and
`sitemap.xml` all say the extensionless form, because a canonical pointing at a
URL that redirects is a canonical pointing at nothing.

Internal `href` links keep `.html` on purpose. They resolve on any static host —
including the plain `npx http-server` in the README — and the redirect is
edge-local and imperceptible. Making the links extensionless would tie the repo
to Cloudflare and break local serving.

### If you ever want zero redirects

Set `"html_handling": "none"` in `wrangler.jsonc`. Then `.html` serves directly
with no hop, and canonicals would need to go back to `.html`.

⚠️ **Untested.** Cloudflare's docs do not state whether `none` still resolves
`/` to `index.html` or `/template/` to its index. If it does not, the homepage
404s. Verify on a preview deployment before putting it on production, and roll
back from **Deployments** if it breaks.
